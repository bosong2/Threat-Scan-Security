# Claude Threat Scan JSON Schema Specification v1.4

본 문서는 `security-report-viewer-claude.html` 뷰어와 호환되는 JSON 출력 규격을 정의한다.

**V1.4 변경사항 (이 문서가 정본)**: finding 객체에 optional `compliance_tags` 추가(KISA·AILLM·TA
컴플라이언스 태그), `repository_summary.ai_agent_scope` optional 추가, `scanner_version` "V2.5".
**V1.4는 V1.3의 strict superset** — 모든 유효한 V1.3 리포트는 V1.4에서도 유효하다(추가 필드는 전부 optional).
상세 태그 규약은 §21(compliance_tags)·`docs/compliance-tagmap-distilled.md`(CTID-D)·
`docs/compliance-tagging-deepdive.md`(CTID-V) 참조. v1.3 문서는 legacy 참조로 보존된다.

**V1.3 변경사항 (승계)**: 컴포넌트 연관관계 그래프·위험 전파, 모델 유효성/진부화 판정, 조치 verdict 체계.
모든 신규 필드는 **optional**이므로 하위 버전 페이로드는 상위 버전에서 그대로 유효하다.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `ai_agent_scope` | string | optional | AI 에이전트 구성요소(`.claude/` 등) 스캔 범위 결정. `"included"` \| `"excluded"`. 부재 = 게이트 미발동(legacy). 번역 비대상(EN/KO 동일). Phase 0(d) 게이트 결과. |
...
| 1.4 | 2026-07-15 | Added: `compliance_tags` (optional, KISA·AILLM·TA), `repository_summary.ai_agent_scope` (optional). scanner_version → V2.5. Strict superset of v1.3 — all v1.3 reports valid. |

---

## 21. compliance_tags (V1.4 신규 — optional)

finding 객체에 컴플라이언스 제어 태그를 부여하는 optional 필드. 상세 방출 규칙은
`docs/compliance-tagmap-distilled.md`(CTID-D, 단계 1–8)·`docs/compliance-tagging-deepdive.md`
(CTID-V, 단계 8.5 검증), 강제 규칙은 `docs/SCHEMA_V1.4_ENFORCEMENT.md` 참조.
...
| Property | Rule |
|----------|------|
| Name | `compliance_tags` — 정확·소문자·복수형. 변형 금지. |
| Type | `array` of `string` |
| Presence | **optional.** 부재 = legacy/untagged(유효). `[]` = "검증했으나 해당 제어 없음"(유효, 부재와 의미 구분). |
| Element pattern | `^#(KISA\|AILLM\|TA)-[A-Z0-9]+(_[A-Z0-9]+)*$` |
| Cardinality | 0–4개 |
| Uniqueness | 배열 내 요소 유일 |
| Ordering | primary(근본원인) 태그 우선 — 순서는 의미론적, 보존 |
| Localization | **없음.** `korean_report`는 배열을 byte-identical로 복사. 태그는 불변 토큰. |
...
**금지 배치**: `scan_metadata`, `repository_summary`, `graph_verdict`,
`prompt_optimization[]`(품질 항목), `recommendations[]`.

### 21.3 Governed namespaces (closed set)

| Namespace | 프레임워크 | 유효 control ID |
|-----------|-----------|-----------------|
| `KISA` | KISA 소프트웨어 보안약점 진단가이드(2021, 고정) | `1_1`–`1_17`, `2_1`–`2_16`, `3_1`–`3_2`, `4_1`–`4_3`, `5_1`–`5_5`, `6_1`–`6_4`, `7_1`–`7_2` |
| `AILLM` | CTID AI/LLM control set v1.1 (OWASP LLM Top 10 2025 / CWE-1426·1427 / MITRE ATLAS) | `8_1`–`8_9` |
| `TA` | Technical/Administrative checklist (정적 평가 가능 부분집합) | `T_A1`, `T_C3`, `T_D1`–`T_D3`, `T_E1`, `T_I1`–`T_I3`, `A_B1` |

검증기는 문법상 유효하나 governed 목록 외 control ID에 **warn**(fail 아님 — 전방 호환). regex 자체는 hard-fail.

### 21.4 AILLM Anchor Registry (normative)

OWASP Top 10 for LLM Applications 2025 / CWE v4.20 / MITRE ATLAS(2025-10) 기준(access 2026-07-15).
...
CWE-250(8_7)·CWE-284(8_8)는 공식 OWASP↔CWE 매핑 부재 시점의 논리적 매핑(assumed) — OWASP 갱신 시 재검토.
