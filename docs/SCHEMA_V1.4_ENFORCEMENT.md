# Schema V1.4 강제 준수 가이드라인 (정본)

> **⚠️ 이 문서는 모든 Security Reports 스킬이 반드시 따라야 하는 절대 규칙입니다.**
> V1.2·V1.3 규칙을 모두 상속하며, V1.4 신규 규칙(§8 compliance_tags, §9 enum 불변)이 추가된다.
> **이 문서가 정본이다.** `SCHEMA_V1.3_ENFORCEMENT.md`는 legacy 참조로만 보존한다 —
> v1.3의 "priority는 등급 번역" 규칙은 §9에서 **폐지**되었다(아래 참조).

---

## 1. 하위 호환성 보장 원칙

**V1.4는 V1.3의 완전한 상위집합이며, V1.3은 V1.2의 상위집합이다.**

- 하위 버전 페이로드는 상위 버전에서 수정 없이 유효하다.
- 신규 필드는 **전부 optional** — 생략 시 파서/뷰어가 오류를 내지 않는다.
- 기존 금지 필드 목록은 그대로 유지되며, V1.4에서 태그 금지 변형(§8.2)이 추가된다.
...
## 8. `compliance_tags` 강제 규칙 (V1.4 신규)

방출 규칙은 `docs/compliance-tagmap-distilled.md`(CTID-D)·`docs/compliance-tagging-deepdive.md`
(CTID-V). 여기서는 강제(enforcement) 규칙만 정의한다.

### 8.1 형식·개수·순서

- 요소 패턴: `^#(KISA|AILLM|TA)-[A-Z0-9]+(_[A-Z0-9]+)*$` — 불일치 시 **hard-fail**.
- 0–4개, 배열 내 유일, primary(근본원인) 우선 순서(순서 보존).
- optional. 부재 = legacy. `[]` = "검증 후 해당 제어 없음"(부재와 의미 구분).
- governed 목록(KISA 49·AILLM 9·TA 10) 외 control ID = **warn**(fail 아님, 전방 호환). regex는 hard-fail.

### 8.2 금지 변형 (스키마 위반)

```text
tags, compliance, kisa_tags, kisaTags, control, controls, control_mapping, owasp_tags → 전부 금지
#kisa-1_1 (소문자 네임스페이스)   → 금지
KISA-1_1 (# 누락)                 → 금지
#KISA-1.1 / #KISA-1-1 (구분자)    → 금지 (control ID 내부는 _ 만)
issue/severity 문자열에 태그 삽입 → 금지
#CWE-89 / #LLM01 (#-태그화)       → 금지 (CWE/OWASP/ATLAS는 description/recommendation prose)
```

### 8.3 배치 규칙

- **허용 8곳**: static_code_findings, binary_analysis_findings, skill_risk_findings,
  agent_policy_findings, sensitive_patterns, relationship_findings, model_validity_findings,
  sbom_analysis 내 finding.
- **금지 배치**(존재 시 fail): scan_metadata, repository_summary, graph_verdict,
  prompt_optimization[], recommendations[].

### 8.4 단계·병합·번역 규칙

- 태그 수정은 **단계 8.5(deepdive)만** 허용. 단계 9–11은 태그 내용 **읽기전용**.
- 병합(단계 9): 태그 verbatim 패스스루. finding **내부** dedup만. finding 간 dedup/재정렬 금지.
- 번역(단계 10): 태그는 불변 토큰. `korean_report`에 **byte-identical** 복사(§9와 동일 원칙).

### 8.5 verdict 화이트리스트 (MASK/KEEP 발명 차단)

모든 finding `verdict` ∈ {`INSTALL_OK`, `REVIEW`, `DISABLE`, `REMOVE`}. `MASK`·`KEEP` 등 임의
값은 스키마 위반이다. "마스킹 필요" 의미는 verdict가 아니라 `recommendation` 텍스트로 서술한다.
병합 단계에서 비정본 값 수신 시 `REVIEW`로 정규화하고 원값 의미를 recommendation에 보존한다.

---

## 9. korean_report enum 불변 규칙 (V1.4 신규 — v1.3 "등급 번역" 폐지)

> **폐지 선언:** v1.3의 "`priority`는 severity처럼 등급 번역(Critical→심각)" 규칙은 **무효**다.
> 아래 enum 필드는 EN/KO 리포트 모두 **영어 원문**을 유지한다(표시 언어 처리는 HTML 템플릿 계층 책임).

### 9.1 번역 금지 enum 필드 (13종)
...
- 예: `"severity": "Critical"` → korean_report에서도 `"Critical"` (`"심각"` 금지).
...
```text
□ 모든 enum 필드(위 13종) 값이 EN/KO 리포트에서 동일 (enum diff = 0)
□ compliance_tags 가 EN/KO byte-identical
□ 서술 텍스트(action/rationale/description 등)만 번역됨
□ 제어 항목명 한국어는 prose에서만 (사전 compliance_controls 참조) — 태그 문자열 치환 금지
```

용어 사전(`security-terms-en-ko.json`)의 `Critical→심각` 매핑은 **서술 문장 내 단어 번역용**이며
JSON enum 값에는 적용하지 않는다.

---

**이 문서는 모든 스킬 개발 및 수정 시 반드시 참조해야 합니다.**

마지막 업데이트: 2026-07-15 (Schema V1.4 — compliance_tags §8, enum 불변 §9)
